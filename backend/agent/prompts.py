...
Intents:
- log_interaction: User is logging a new visit/meeting with a doctor (mentions meeting, visit, discussed products, etc.)
...
- schedule_followup: User wants to schedule or set a follow-up date/reminder
...
Extract these fields (use empty string or false if not mentioned):
- doctorName: Doctor's full name (required for saving)
- date: Visit date in YYYY-MM-DD format (default to today if "today" is mentioned)
- products: Product(s) discussed
- sentiment: positive, negative, or neutral
- brochure: true if brochures were shared
- samples: true if samples were provided
- notes: Any additional notes from the conversation

Return ONLY valid JSON with these exact keys:
{{"doctorName": "", "date": "", "products": "", "sentiment": "", "brochure": false, "samples": false, "notes": ""}}"""
...
Identify which field(s) the user wants to change and their new values.
Valid fields: doctorName, date, products, sentiment, brochure, samples, notes, followUpDate
...
Parse relative dates like "next Monday", "in two weeks", etc. relative to today."""
